  ...
  return (
    <div
      ...
      className={cn(
        ...
        checked ? "bg-emerald-500" : "bg-white/10 border border-white/20"
      )}
    >
      <div 
        className={cn(
...
const NativeSlider = ({ value, min, max, onChange }: { value: number, min: number, max: number, onChange: (v: number) => void }) => {
  ...
  return (
    <div className="relative w-full h-1.5 bg-white/20 rounded-full shadow-inner flex items-center">
      <div 
        className="absolute left-0 h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)] pointer-events-none transition-all duration-75" 
      ...
      />
      <div 
        className="absolute w-5 h-5 bg-white rounded-full shadow-md pointer-events-none transition-all duration-75 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)] border border-emerald-500/20" 
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent showCloseButton={false} className="sm:max-w-[440px] w-full p-0 border border-white/20 bg-black/40 backdrop-blur-2xl shadow-2xl rounded-3xl z-[100] h-[90vh] sm:h-[80vh] flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col gap-6">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive ? "bg-emerald-500 text-black shadow-md font-semibold" : "text-white/50 hover:text-white hover:bg-white/5"
                      )}
                    >
                      {tip}
                    </button>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <BatteryMedium size={18} className="text-emerald-500" />
                ...
                <div className="flex items-center">
                  ...
                  <span className="w-8 text-emerald-500 font-bold text-[15px] text-center">{varisSarj}</span>
                </div>
              </div>
              <NativeSlider 
                value={varisSarj} 
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input 
                  ...
                  className="w-full bg-white/5 backdrop-blur-sm rounded-xl px-4 py-3.5 text-white/70 text-sm border border-white/10 focus:outline-none focus:border-emerald-500/50 transition-all [color-scheme:dark]"
                />
                <input 
                  type="time" 
                 ...
                 {(['HPC', 'DC', 'AC'] as const).map(tip => (
                   <button 
                     ...
                     className={cn(
                       ...
                       sarjTercipi === tip 
                         ? "bg-emerald-500 text-black shadow-[0_0_15px_rgba(16,185,129,0.3)] border-emerald-500" 
                ...
                {[
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn("text-[12px] font-medium text-center transition-colors", isActive ? "text-emerald-400" : "text-white/50")}>{item.id}</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div className="p-5 shrink-0 bg-black/40 border-t border-white/10 backdrop-blur-xl z-[50]">
           <button onClick={onClose} className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-4 rounded-2xl transition-all duration-200 active:scale-[0.98] shadow-[0_0_15px_rgba(16,185,129,0.3)]">
             Ayarları Uygula
           </button>
        </div>
